...
function PipelineDashboard({ runs, throughputData }: PipelineDashboardProps) {
          ...
          {runs.length === 0 ? (
            ...
            runs.map((run) => (
              ...
              >
                ...
                {/* Resource Usage (FIXED: cpuUsage & memoryUsage fallback) */}
                <div className="grid grid-cols-2 gap-4 mt-3">
                  <div className="flex items-center gap-2">
                    <span className="text-xs text-muted-foreground">CPU:</span>
                    <div className="flex-1 bg-background rounded-full h-2">
                      <div
                        className={`h-2 rounded-full ${((run.cpuUsage ?? 0) > 80)
                          ? 'bg-red-500'
                          : (run.cpuUsage ?? 0) > 60
                          ? 'bg-yellow-500'
                          : 'bg-green-500'}`}
                        style={{ width: `${run.cpuUsage ?? 0}%` }}
                      />
                    </div>
                    <span className="text-xs text-white">{run.cpuUsage ?? 0}%</span>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex-1 bg-background rounded-full h-2">
                      <div
                        className={`h-2 rounded-full ${((run.memoryUsage ?? 0) > 400)
                          ? 'bg-red-500'
                          : (run.memoryUsage ?? 0) > 300
                          ? 'bg-yellow-500'
                          : 'bg-green-500'}`}
                        style={{ width: `${((run.memoryUsage ?? 0) / 512) * 100}%` }}
                      />
                    </div>
                    <span className="text-xs text-white">{run.memoryUsage ?? 0}MB</span>
                  </div>
                </div>
                    ...
                    {run.errorMessages.map((error, idx) => (
                      <p key={idx} className="text-xs text-red-400 flex items-center gap-2">
                        <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none">
                          ...
                          <path d="M10.29 3.86L1.82 18a2 2 0 001.71 3h17.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z" stroke="currentColor" strokeWidth="1.5" />
                        </svg>
                        <span>{error}</span>
                      </p>
                    ))}
