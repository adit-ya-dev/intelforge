...
export default function OnboardingWizard({ onClose }: OnboardingWizardProps) {
  ...
  const getStepContent = () => {
    switch (currentStep) {
      case 1: return <DomainSelection />;
      case 2: return <ConnectorConfiguration />;
      case 3: return <WatchlistBuilder />;
      case 4: return <AlertSetup />;
      default: return null;
    }
  };
              ...
              <div key={index} className="flex-1 relative">
                <div
                  className={`h-2 rounded-full transition-colors ${
                    index + 1 <= currentStep ? 'bg-blue-600' : 'bg-zinc-200 dark:bg-zinc-800'
                  }`}
                />
              </div>
            ))}
