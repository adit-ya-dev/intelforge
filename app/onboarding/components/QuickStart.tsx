// app/onboarding/components/QuickStart.tsx
  ...
  const detailedSteps = [
    {
      ...
      content: 'Set up your organization details and preferences to personalize your experience.',
      tasks: ['Add organization information', 'Select your role', 'Set notification preferences']
    },
    {
      title: 'Step 2: Connect Data Sources',
      content: 'Enable connectors to start ingesting technology intelligence data.',
      tasks: ['Enable USPTO patent connector', 'Configure arXiv feed', 'Set up funding data sources']
    },
  ];

  // Logic handled: Removed color-specific classes, now using uniform blue style
  const cardClass = "bg-white dark:bg-black border border-zinc-200 dark:border-zinc-800 rounded-lg";
  const hoverClass = "hover:border-blue-600 dark:hover:border-blue-500 hover:bg-blue-50 dark:hover:bg-blue-900/10";
                    ...
                    <div className="space-y-2">
                      {step.tasks.map((task, taskIndex) => (
                        <label key={taskIndex} className="flex items-center gap-3 cursor-pointer">
                          <input type="checkbox" className="w-4 h-4 bg-white dark:bg-black border border-zinc-300 dark:border-zinc-700 rounded text-blue-600" />
    ...
    <div className="bg-white dark:bg-black rounded-xl border border-zinc-200 dark:border-zinc-800 p-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {quickActions.map((action) => (
          ...
          >
            <div className="text-blue-600 dark:text-blue-500 mb-3">
