// app/onboarding/components/SampleDataExplorer.tsx

"use client";

import { useState } from 'react';

// 1. Define the data structure locally to ensure the component works standalone
...
// 2. Define the mock data locally
        ...
        {activeView === 'searches' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {searches.map((search) => (
              <div
                key={search.id}
                className="bg-white dark:bg-black border border-zinc-200 dark:border-zinc-800 rounded-lg p-4 hover:border-blue-600 dark:hover:border-blue-500 transition-colors group"
              >
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <span className="text-xs text-blue-600 dark:text-blue-400 font-medium uppercase tracking-wide">{search.category}</span>
                    <h3 className="font-medium text-black dark:text-white mt-1 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{search.query}</h3>
                  </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <button className="px-3 py-1 text-xs bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 border border-blue-200 dark:border-blue-900/50 rounded hover:bg-blue-100 dark:hover:bg-blue-900/30 transition-colors">
                    Try Search
                  </button>
                </div>
              </div>
            ...
            <div className="bg-white dark:bg-black border border-zinc-200 dark:border-zinc-800 rounded-lg p-4">
              ...
              <div className="h-48 flex items-center justify-center text-zinc-300 dark:text-zinc-700">
                <svg className="w-full h-full" viewBox="0 0 300 150">
                  <path
                    d="M 10 140 Q 50 140 100 75 T 290 10"
                    stroke="currentColor"
                    strokeWidth="2"
                    fill="none"
                    opacity="0.3"
                  />
                  <path
                    d="M 10 140 Q 50 140 100 75 T 200 30"
                    stroke="#2563EB"
                    strokeWidth="3"
                    fill="none"
                  />
              ...
              <div className="h-48 flex items-end justify-between gap-2 px-4">
                {[40, 60, 45, 70, 85, 95, 80].map((height, i) => (
                  <div key={i} className="flex-1 group">
                    <div
                      className="bg-blue-600/80 dark:bg-blue-600/60 rounded-t group-hover:bg-blue-600 transition-colors"
                      style={{ height: `${height}%` }}
                    />
                  </div>
                ))}
