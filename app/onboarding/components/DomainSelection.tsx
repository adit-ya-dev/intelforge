// app/onboarding/components/DomainSelection.tsx
...
export default function DomainSelection() {
  ...
  const handleToggleDomain = (domainId: string) => {
    setDomains(domains.map(d => 
      d.id === domainId ? { ...d, selected: !d.selected } : d
    ));
  };
        ...
        <div className="flex-1 max-w-md">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 bg-white dark:bg-black border border-zinc-200 dark:border-zinc-800 rounded-lg text-black dark:text-white placeholder-zinc-400 focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
          </div>
        </div>
        <div className="text-right">
          <p className="text-sm text-zinc-500 dark:text-zinc-400">Selected</p>
          <p className="text-2xl font-bold text-black dark:text-white">{selectedCount}/{domains.length}</p>
        </div>
      </div>
